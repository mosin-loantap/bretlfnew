                            ...
                            <form id="personalLoanDetailsForm">
                                ...
                                <div class="form-group">
                                    <div class="input-group">
                                        ...
                                        <input type="file" id="bankStatement" class="form-control" accept=".pdf,.jpg,.jpeg,.png" required>
                                    </div>
                                    <small class="text-muted">Upload your bank statement (PDF, JPG, PNG - Max 10MB)</small>
                                </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            personalLoanDetailsForm.addEventListener('submit', function(e) {
                ...
                // Prepare complete application data for Salesforce
                ...
                .then(data => {
                    ...
                    if (data.status === 'success') {
                        alert('Application submitted successfully to Salesforce! Lead ID: ' + data.lead_id);
                        console.log('Salesforce Lead created:', data);
                        
                        // Upload bank statement if provided
                        const bankStatementFile = document.getElementById('bankStatement').files[0];
                        if (bankStatementFile && data.lead_id) {
                            uploadBankStatement(data.lead_id, bankStatementFile);
                        }
                    } else {
                        alert('Application submitted locally but failed to sync with Salesforce: ' + data.message);
                ...
                .catch(error => {
                    ...
                    alert('Application data saved locally. Network error occurred while syncing with Salesforce.');
                });
            ...
            // Function to upload bank statement to Salesforce
            function uploadBankStatement(leadId, file) {
                const formData = new FormData();
                formData.append('file', file);
                formData.append('title', 'Bank Statement');
                formData.append('description', 'Bank statement uploaded with loan application');
                
                showLoader('Uploading Bank Statement...', 'Attaching document to your application');
                
                fetch(`/api/salesforce/lead/${leadId}/upload`, {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || ''
                    },
                ...
                .then(data => {
                    hideLoader();
                    if (data.status === 'success') {
                        alert('Bank statement uploaded successfully to Salesforce!');
                        console.log('File uploaded:', data);
                    } else {
                        alert('Failed to upload bank statement: ' + data.message);
                ...
                .catch(error => {
                    ...
                    alert('Failed to upload bank statement due to network error.');
                });
            }
        });
    </script>
